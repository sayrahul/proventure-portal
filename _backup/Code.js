...
function addTask(payload) {
    ...
    // --- AUTO-PROVISION PROJECT HANDLING ---
    if (payload.ProjectID === 'AUTO') {
        const projectsSheet = getSheet('Projects');
        const allProjects = projectsSheet.getDataRange().getValues();
        // Skip header
        let userProjectId = null;
        // Verify if user already has a project (col index 2 for ClientID, 0 for ProjectID)
        // Adjust indices based on your actual sheet columns
        // Assuming: ProjectID (0), ProjectName (1), ClientID (2)...

        for (let i = 1; i < allProjects.length; i++) {
            if (allProjects[i][2] == payload.CreatedBy) {
                userProjectId = allProjects[i][0];
                break;
            }
        }
        ...
        if (!userProjectId) {
            // Create New Project
            userProjectId = 'PRJ-' + Math.floor(1000 + Math.random() * 9000);
        ...
        payload.ProjectID = userProjectId;
    }
    // ---------------------------------------
